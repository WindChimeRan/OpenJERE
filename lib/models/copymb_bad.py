...
class CopyMB(nn.Module):
    def __init__(self, hyper) -> None:
        ...
        if hyper.cell_name == "gru":
            ...
            self.decoder = nn.GRU(
                hyper.emb_size, hyper.hidden_size, bidirectional=False, batch_first=True
            )
        ...
        self.rel_linear_b = nn.Linear(hyper.hidden_size, len(self.relation_vocab))
    ...
    def forward(self, sample, is_train: bool) -> Dict[str, torch.Tensor]:
        ...
        hidden_idx = torch.tensor(
            list(map(lambda x: x - 1, length)), dtype=torch.long
        ).cuda(self.gpu)
        hidden_idx = (
            torch.zeros_like(tokens).scatter_(1, hidden_idx.unsqueeze(1), 1).bool()
        )

        h = o[hidden_idx]
        h = (
            h.unsqueeze(1)
            .expand(-1, self.hyper.max_text_len, -1)
            .contiguous()
            .view(1, -1, self.hyper.hidden_size)
            .contiguous()
        )
    ...
    def _decode_step(self, t: int, decoder_state, decoder_input, o):
        ...
        if t % 2 == 0:
            ...
            output_logits = self.entity_linear_2(
                self.activation(self.entity_linear_1(self.activation(output_logits)))
            ).squeeze()
